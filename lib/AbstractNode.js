...
//   emit contentAdded and contentRemoved. The new refactored SpaceView
...
p._addChild = function (abstractNode) {
  // To be called from abstractNode.setParent().
  // If called from anywhere else, ensure cyclic relationships are detected.
  this._children[abstractNode.id] = abstractNode
}

p._removeChild = function (abstractNode) {
  // To be called from abstractNode.setParent().
  // Precondition: abstractNode is in space
  delete this._children[abstractNode.id]
}
