...
const Sensor = function (element, handlers, opts) {
  ...
  //   opts
  //     delegation
  //       optional string. One of following values:
  //         'none': no element is allowed as target. Basically this
  //         .. lets events flow through the sensor untouched.
  //         'self': only catch events with the captured element as target.
  //         .. Events from child elements or their successors stay untouched.
  //         'affine': only catch events with affine target.
  //         .. Events from non-affine targets stay untouched.
  //         'handles': only catch events from elements marked as handles.
  //         .. Handles can be affine or non-affine.
  //         'all': catch all events relevant for gesture capturing, regardless
  //         .. of their depth or type. This effectively prevents all default
  //         .. behavior interaction in child elements and their successors.
  //     preventDefault
  ...
  this.onpointerdown = function (ev) {
    ...
    if (!self.isEventConsumable(ev)) { return }
...
proto.isEventConsumable = function (ev) {
  // Test if event has a valid target for the sensor.
  // Target is valid if event has not yet been prevented
  // and conforms to the delegation rule.
  //
  // Parameters:
  //   ev
  //
  ...
  // Shortcircuit if target not selected in delegation.
  ...
  if (delegation === 'affine') {
    if (ev.target.affine) {
      return true
    }
    return false
  }
  if (delegation === 'handles') {
    // if (ev.target === this.element) {
    //   // Consider the captured element an implicit handle.
    //   return true
    // }
    if (ev.target.dataset.affineAction === 'handle') {
      return true
    }
    // Try find greedy handles
    let t = ev.target
    while (t && t !== this.element) {
      if (t.dataset.affineAction === 'greedy-handle') {
        return true
      }
      t = t.parentElement
    }
  ...
  throw new Error('Unknown delegation option value.') // would be a bug
}
