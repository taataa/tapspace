...
p.mount = function (htmlContainer) {
  ...
  // Without "overflow: hidden", if transformed elements get outside the initial
  // viewport, Chrome on Android increases the dimensions of the document.
  this._el.style.position = 'relative'
  this._el.style.overflow = 'hidden'
  this._el.style.display = 'block'

  // Init size in space.
  this.resize([this._el.clientWidth, this._el.clientHeight])
...
p._resizeElementOf = function (spaceRectangle) {
  ...
  if (this._elements.hasOwnProperty(spaceRectangle.id)) {
    ...
    if ('getSize' in spaceRectangle) {
      wh = spaceRectangle.getSize()
...
p._transformElementOf = function (spaceNode) {
  ...
  if (this._elements.hasOwnProperty(spaceNode.id)) {
    el = this._elements[spaceNode.id]
    gt = spaceNode.getGlobalTransform().T
