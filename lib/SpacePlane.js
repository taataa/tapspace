...
// SpacePlane
//
// A SpacePlane represents a coordinate system. It includes
// methods to transform the system.
//
// Note: In v3 and in alpha stages of v4 there was
//   SpacePlane and SpaceTransformer that were then
//   merged together to form the new SpacePlane.
...
var SpacePlane = function () {
  ...
  this.on('removed', function (self, newParent, oldParent) {
    ...
    // If SpaceNode was just moved to a new parent, preserve
...
p.setGlobalLocalTransform = function (itr) {
  ...
  // want the planes be visually double in size, you can call
  // setGlobalLocalTransform with 2x scaling for each plane.
  //
  // Parameters:
  //   itr
  //     InvariantTransform
...
p.snap = function (pivot, igrid) {
  // Snap the plane to the given InvariantGrid at pivot
  //
  // Parameters
  //   pivot
  //     InvariantVector
  ...
  //
  // Dev. note
  //   The same can be done with InvariantGrid with:
  //   var itr =  new InvariantTransform(this._T, this._parent)
