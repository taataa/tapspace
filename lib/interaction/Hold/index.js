// TODO Provide various hold animations.
...
const HoldGesture = function (source, target, options) {
  ...
  // Parameters:
  //   source
  //     a Component. The hold input source.
  //   target
  //     a Component. The target for the hold effects.
  //     .. The target will emit hold events.
  //   options
  //     holdDuration
    ...
    if (self.bound && !self.canceled) {
      ...
      if (ev.travel > self.options.maxTravel) {
        ...
        if (self.heldEnough) {
          ...
          self.target.emit('holdend', ev)
        } else {
          // Cancel hold
          self.canceled = true
          clearInterval(self.progressTimer)
          self.target.emit('holdcancel', ev)
        }
      }
    }
  }
