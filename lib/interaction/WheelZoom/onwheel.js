module.exports = (viewport) => {
  ...
  return (ev) => {
    ...
    if (viewport.isPerspective()) {
      ...
      // const delta = camera.getVectorTo(center)
      ...
      let gestureCenterOnTarget // is a Point.
      ...
      if (ev.target === viewport || ev.target === viewport.getSpace()) {
        ...
        gestureCenterOnTarget = gestureCenter.projectTo(possibleTarget, camera)
      } else {
        // We aim to move camera closer to the point of gesture target.
        gestureCenterOnTarget = gestureCenter.projectTo(ev.target, camera)
      }

      // We want to scale the length of the vector from camera to target.
      const delta = camera.getVectorTo(gestureCenterOnTarget)
      ...
      // Orthogonal
      viewport.scaleBy(factor, gestureCenter)
    }
