module.exports = (viewport) => {
  ...
  return (ev) => {
    ...
    if (ev.target === viewport || ev.target === viewport.getHyperspace()) {
      ...
      const possibleTarget = viewport.findNearRay(pivot)
      if (possibleTarget) {
    ...
    // DEBUG
    // const scale = selectedTarget.getScale().transitRaw(viewport)
    // console.log('scale on viewport', scale)
    // console.log('measured dilation', viewport.measureDilation(selectedTarget))
