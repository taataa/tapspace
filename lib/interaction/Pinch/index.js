const defaultApplicator = require('./defaultApplicator')
...
const PinchGesture = function (source, target, options) {
  ...
  //           .. Default is null.
  //     applicator
  //       optional function (source, target, ev). The function applies
  //       .. the pinch transformation to an element, preferrably the target.
  //       .. The applicator is called on each gesturemove.
  //       .. Default applicator calls target.transformBy(ev.delta, ev.center)
  //
  ...
  if (typeof options.applicator === 'function') {
    this.options.applicator = options.applicator
  } else {
    this.options.applicator = defaultApplicator
  }
...
proto.bind = function () {
  ...
  this.ongesturemove = (ev) => {
    ...
    self.options.applicator(self.source, self.target, ev)
