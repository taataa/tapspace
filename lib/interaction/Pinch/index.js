...
proto.bind = function () {
  ...
  this.ongesturemove = (ev) => {
    ...
    // Transform is locationless. A center point gives it a location.
    // Alternatively, think that the transform will be applied at a point.
    const center = self.options.freedom.center
    if (center) {
      console.log('center', center.point)
      console.log('freedom', self.capturer.getFreedom())
      console.log('ev.delta', ev.delta.helm)
      self.target.transformBy(ev.delta, center)
    } else {
      // TODO use gesture center
