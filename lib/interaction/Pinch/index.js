...
const PinchGesture = function (source, target, options) {
  ...
  // Parameters:
  //   source
  //     an InteractiveComponent. Get gesture input from this component.
...
proto.bind = function () {
  ...
  this.ongesturestart = (ev) => {
    ...
    // Detect viewport being the source.
    // Interacting with background is problematic due to uncertain depth.
    // Therefore set navigation basis to nearest item.
    if (self.source.isViewport) {
      const basis = self.source.findNearestProjected(ev.mean)
      self.source.setNavigationBasis(basis)
    }
