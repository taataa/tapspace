...
proto.bind = function () {
  ...
  this.onresize = (ev) => {
    ...
    // Anchor relative to the component
    const relAnchor = this.options.relativeAnchor

    const prevSize = ev.previousSize.getRaw()
    const newSize = ev.size.getRaw()

    // Find the anchor point when the component had the previous size.
    const oldAnchor = ev.target.at(
      prevSize.w * relAnchor.rx,
      prevSize.h * relAnchor.ry
    )
    // Compare it to the similar anchor point with the new size.
    const newAnchor = ev.target.at(
