...
const SlideGesture = function (source, target, options) {
  ...
  // Parameters:
  //   source
  //     a Component. The slide input source.
  //   target
  //     a Component. The target for the slide effect.
  //     .. The target will emit slide events.
  //   options
  //     direction
  ...
  if (typeof options.direction === 'object') {
    ...
    this.options.direction = source.getDirection(0)
  }
...
proto.bind = function () {
  ...
  this.ongesturestart = (ev) => {
    ...
    if (self.bound) {
      if (ev.travel >= self.options.minTravel) {
        self.begun = true
        self.target.emit('slidestart', ev)
      }
    }
  }
  this.ongesturemove = (ev) => {
    if (self.bound) {
      if (self.begun) {
        ...
        self.target.emit('slidemove', ev)
        self.target.emit('slide', ev)
      } else {
        if (ev.travel >= self.options.minTravel) {
          self.target.translateBy(ev.delta) // TODO include travel this far?
          self.begun = true
          self.target.emit('slidestart', ev)
        }
      }
  ...
  this.ongestureend = (ev) => {
    if (self.bound && self.begun) {
      self.begun = false
      self.target.emit('slideend', ev)
    }
  }
  this.ongesturecancel = (ev) => {
    if (self.bound && self.begun) {
      self.begun = false
      self.target.emit('slidecancel', ev)
    }
  }
