...
p.fitScale = function (ipa) {
  ...
  if (ir > r) {
    ...
    domain = new Path([
      ...
      new Vector(selfBounds.get(1).x, selfBounds.get(1).y + hh), // B'
      new Vector(selfBounds.get(2).x, selfBounds.get(2).y - hh), // C'
      new Vector(selfBounds.get(3).x, selfBounds.get(3).y - hh)  // D'
    ])
  } else {
    ...
    domain = new Path([
      ...
      new Vector(selfBounds.get(1).x - wh, selfBounds.get(1).y), // B'
      new Vector(selfBounds.get(2).x - wh, selfBounds.get(2).y), // C'
      new Vector(selfBounds.get(3).x + wh, selfBounds.get(3).y)  // D'
    ])
  }

  idomain = new InvariantPath(domain, this)
  irange = new InvariantPath(ipaBounds, this)
...
p.fitSize = function (ipa) {
  ...
  // Translate to center
  this.translate(this.atNW(), new InvariantVector(nw, this))
...
p.getSize = function () {
  ...
  return new InvariantVector(this._diag, this)
}
