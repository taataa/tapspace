...
var Recognizer = function (element, plane, view, emitter, mode) {
  // Create a Recognizer
  ...
  var onMove = function (prevPointers, nextPointers) {
    ...
    if (self.view === self.plane) {
      ...
      // Why? See below.
      newT = plane._T.multiplyRight(tr.inverse())
      plane.setLocalTransform(newT)
    } else {
      ...
      //  plane.transformBy(itr)
      // <=> next.T = T * tr
      newT = plane._T.multiplyRight(tr)
      plane.setLocalTransform(newT)
    }
  ...
  this.recognizer = new Sensor(element, {
    start: onStart,
    move: onMove,
    end: onEnd
  }, this.mode.propagate)
