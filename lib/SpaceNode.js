...
p.setParent = function (newParent) {
  ...
  // Old parent at ancs[0]
  ancs = this.getAncestors()
  ...
  if (ancs.length === 0) {
    if (newParent === null) {
      ...
      pancs = newParent.getAncestors()

      this.emit('added', this, newParent, null)
      newParent.emit('contentAdded', this, newParent, null)
      for (i = 0; i < pancs.length; i += 1) {
        pancs[i].emit('contentAdded', this, newParent, null)
      }
    }
  } else {
    if (newParent === null) {
      ...
      this._parent = null
      ancs[0]._removeChild(this)

      this.emit('removed', this, null, ancs[0])

      for (i = 0; i < ancs.length; i += 1) {
        ancs[i].emit('contentRemoved', this, null, ancs[0])
      }
    } else {
      // From child to child.
      // Remove and add.
      ...
      this.emit('removed', this, newParent, ancs[0])
      for (i = 0; i < ancs.length; i += 1) {
        ancs[i].emit('contentRemoved', this, newParent, ancs[0])
      }

      this.emit('added', this, newParent, ancs[0])
      newParent.emit('contentAdded', this, newParent, ancs[0])
      for (i = 0; i < pancs.length; i += 1) {
        pancs[i].emit('contentAdded', this, newParent, ancs[0])
      }
    }
  }
}
