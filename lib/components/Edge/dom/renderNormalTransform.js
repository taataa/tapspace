...
module.exports = function (edge, attractor) {
  ...
  const vi = vec3.unit(vab) // along edge
  const vj = vec3.unit(vec3.cross(vab, vac)) // cross edge
  const vk = vec3.unit(vec3.cross(vi, vj)) // edge normal

  // Origin point
  const x = tr.x
  const y = tr.y
  const z = -tr.z // browsers have negated z
