...
module.exports = function (position, rotation, scale, anchor) {
  ...
  if (position.basis) {
    ...
    const prh = position.basis.getTransitionTo(this)
    const tran = geom.plane3.compose(this.plane, prh)
  ...
  let r = rotation
  if (rotation.basis) {
    const prh = rotation.basis.getTransitionTo(this)
    const tran = geom.plane3.compose(this.plane, prh)
  ...
  let s = scale
  if (scale.basis) {
    const prh = scale.basis.getTransitionTo(this)
    const tran = geom.plane3.compose(this.plane, prh)
  ...
  const panc = geom.vec3.transitFrom(anc, this.plane)
  ...
  const pz = pos.z - pancz

  // Now we have everything to form the projection.
  this.plane = { a: a, b: b, x: px, y: py, z: pz }
