...
module.exports = function (params) {
  ...
  for (let i = 0; i < params.sources.length; i += 1) {
    ...
    if (p.basis) {
      ...
      domain.push(geom.point2.transitFrom(p.point, pr)) // to point2
    } else {
  ...
  for (let i = 0; i < params.targets.length; i += 1) {
    ...
    if (p.basis) {
      const pr = p.basis.getTransitionTo(this)
      range.push(geom.point2.transitFrom(p.point, pr)) // to point2
    } else {
      // Assume the point is already on the plane.
  ...
  if (params.center) {
    ...
    if (c.basis) {
      ...
      center = geom.point2.transitFrom(c.point, pr) // c becomes plain {x,y}
    } else {
      // Assume the point is already on the plane.
  ...
  let angle = null
  if (params.angle) {
    let dir = params.angle
    if (dir.basis) {
      const pr = dir.basis.getTransitionTo(this)
      const dirOnThis = geom.dir2.transitFrom(dir.dir, pr)
      angle = geom.dir2.toAngle(dirOnThis)
    } else {
