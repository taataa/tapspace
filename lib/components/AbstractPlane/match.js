...
module.exports = function (params) {
  ...
  for (let i = 0; i < params.sources.length; i += 1) {
    ...
    if (p.basis) {
      ...
      domain.push(geom.point2.transitFrom(p, pr)) // p becomes plain {x,y}
    } else {
  ...
  for (let i = 0; i < params.targets.length; i += 1) {
    ...
    if (p.basis) {
      const pr = p.basis.getTransitionTo(this)
      range.push(geom.point2.transitFrom(p, pr)) // p becomes plain {x,y}
    } else {
      // Assume the point is already on the plane.
  ...
  if (params.center) {
    ...
    if (c.basis) {
      ...
      center = geom.point2.transitFrom(c, pr) // c becomes plain {x,y}
    } else {
      // Assume the point is already on the plane.
  ...
  if (params.angle) {
    ...
    if (dir.basis) {
      ...
      angle = geom.dir2.transitFrom(dir.r, pr) // becomes plain number
    } else {
