...
module.exports = function (opts) {
  ...
  // or replaced the component.plane object.
  //
  // Parameters:
  //   opts
  //     projection
  //       optional plane2 transition to be used instead of this.plane.
  ...
  const g = { a: 1, b: 0, x: anc.x, y: anc.y, z: 0 }
  const ig = { a: 1, b: 0, x: -anc.x, y: -anc.y, z: 0 }
  // Adjusted projection. TODO OPTIMIZE avoid function calls.
  plane = geom.plane3.compose(ig, plane3.compose(plane, g))

  if (plane.z) {
    applyTransform3d(this.element, plane)
  } else {
    applyTransform(this.element, plane)
  }
}
