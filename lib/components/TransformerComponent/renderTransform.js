...
module.exports = function (alt) {
  // @TransformerComponent:renderTransform(alt)
  //
  ...
  // Project the plane towards parent anchor, if any.
  const parent = this.getParent()
  if (parent && parent.isSpace) {
    const viewport = parent.getViewport()
    const camera = viewport.atCamera().transitRaw(parent)
    // const camera = parent.atAnchor().offset(0, 0, -300).getRaw()
    const parentTran = plane3.IDENTITY
    tran = plane3.projectToPlane(tran, parentTran, camera)
  }
