...
module.exports = function (tr, origin) {
  ...
  // Transform the children, not the hyperspace itself.
  const spaces = this.getChildren()

  // Transform each space silently.
  // This looks like the hyperspace is transformed or that viewport
  // is inversely transformed.
  for (let i = 0; i < spaces.length; i += 1) {
    const space = spaces[i]
    space.tran = plane3.compose(trPassive, space.tran)
