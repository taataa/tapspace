...
module.exports = function (frame, options) {
  ...
  tran = plane2.scaleBy(tran, scaleOrigin, scaleFactor)
  ...
  const scaledFrameSize = size2.scaleBy(frameSize, scaleFactor)
  const offsetX = (portSize.w - scaledFrameSize.w) / 2
  const offsetY = (portSize.h - scaledFrameSize.h) / 2

  tran = plane2.translateBy(tran, { x: offsetX, y: offsetY })
