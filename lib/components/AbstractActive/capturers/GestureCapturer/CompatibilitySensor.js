...
module.exports = function Sensor (element, handlers, opts) {
  ...
  const onTouchCancel = function (ev) {
    // A known touch point cancels.
    ...
    if (_started) {
      ...
      const cts = ev.changedTouches
      for (let i = 0; i < cts.length; i += 1) {
        // We assume that cancelled pointers are not moved from last touchmove
        // although the coordinates could be accessed via
        // ev.changedTouches[i].pageX and .pageY.
        delete nextPointers[cts[i].identifier]
      }
      ...
      if (Object.keys(nextPointers).length < 1) {
        ...
        // not the empty next pointers.
        _handlers.oncancel(_currPointers)
      }
  ...
  _el.addEventListener('touchcancel', onTouchCancel)
  ...
  _listeners.touchcancel = onTouchCancel
