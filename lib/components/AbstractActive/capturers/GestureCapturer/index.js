...
//       In 2.0.0-alpha.0 we tried to project the pivot onto page and do
//       the estimation in page coordinates. However, the resulting
//       transformation was offsetted and would have needed projecting back
//       to our viewport. This felt sketchy and therefore we decided to
//       project the pointer page coordinates onto the viewport instead.
//
//
...
const projectFreedom = require('./projectFreedom')
...
const GestureCapturer = function (component, options) {
  ...
  const onStart = (firstPointers) => {
    ...
    pointersOnRoot = projectPointers(firstPointers, root)

    // TODO Track where pointers appear
    // TODO entryPoints = projectPointers(firstPointers, root, component)
  ...
  const onMove = (prevPointers, nextPointers) => {
    ...
    const nextPointersOnRoot = projectPointers(nextPointers, root)
    ...
    // move the viewport.
    const freedomOnRoot = projectFreedom(this.freedom, root)
