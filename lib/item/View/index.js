...
p.getElementByAbstractNode = function (abstractNode) {
  // Get HTML element representation of the space node.
  // If abstractNode is the view itself, return the container.
...
p.getAbstractNodeByElementId = function (id) {
...
p._removeElementOf = function (abstractNode) {
  ...
  if (n === this || n === this._parent) {
    // When AbstractNode is the Space or SpaceViewHTML itself,
...
p._resizeElementOf = function (AbstractRectangle) {
  var el, wh
  wh = AbstractRectangle.getLocalSize()
  el = this._elements[AbstractRectangle.id]
  el.style.width = wh.x + 'px'
  el.style.height = wh.y + 'px'
}

p._transformElementOf = function (abstractNode) {
  ...
  el = this._elements[abstractNode.id]
